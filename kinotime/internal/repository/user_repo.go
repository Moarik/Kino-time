...
func (repo *UserRepository) GetUserIdByName(ctx context.Context, name string) (string, error) {
	if err := repo.DB.Ping(); err != nil {
		log.Println("Database is closed or unreachable:", err)
	...
	var id string
	err := repo.DB.QueryRowContext(ctx, "SELECT id FROM users WHERE username = $1", name).Scan(&id)
	if err != nil {
		return "", fmt.Errorf("user not found")
	}
	return id, nil
